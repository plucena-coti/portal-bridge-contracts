...
console.log('🚀 Public Balance Test Loading...');

test.describe('Privacy Bridge - Public COTI Balance on COTI Testnet', () => {
    ...
    test('should connect wallet and display PUBLIC COTI balance', async () => {
        console.log('');
        console.log('========================================');
        console.log('🎬 PUBLIC BALANCE TEST STARTED');
        console.log('========================================');
        ...
        try {
            ...
            // Step 6: Check balance (no reload needed if already connected)
            console.log('');
            console.log('📍 STEP 6: Check Balance');
            await page.bringToFront();

            // Only reload if the balance section isn't already visible
            let publicTokensSection = page.locator('section[aria-labelledby="public-tokens-heading"]');
            let isSectionVisible = await publicTokensSection.isVisible({ timeout: 3000 }).catch(() => false);

            if (!isSectionVisible) {
                console.log('  → Balance section not visible, reloading page...');
                await page.reload();
                await page.waitForLoadState('networkidle');
                await page.waitForTimeout(3000);
            } else {
                console.log('  ✅ Balance section already visible');
            }

            publicTokensSection = page.locator('section[aria-labelledby="public-tokens-heading"]');
            await expect(publicTokensSection).toBeVisible({ timeout: 15000 });
            console.log('✅ Public tokens section visible');
            ...
            const ariaLabel = await cotiButton.getAttribute('aria-label');
            console.log('📊 Token row:', ariaLabel);

            const balanceMatch = ariaLabel?.match(/balance\s+([\d,.]+)/);
            ...
            if (balanceMatch) {
                ...
                console.log('');
                console.log('========================================');
                console.log('✅ SUCCESS: Public Balance Verified!');
                console.log('========================================');
            }
            ...
            console.error('');
            console.error('========================================');
            console.error('❌ PUBLIC BALANCE TEST FAILED');
            console.error('========================================');
            console.error(error);
            throw error;
        }
