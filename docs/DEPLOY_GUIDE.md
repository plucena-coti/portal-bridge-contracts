...
VITE_DEFAULT_NETWORK_ID=0x6c11a0
VITE_USE_ENCRYPTED_DEPOSITS=true
```
...
**Before running the script make sure public token addresses are correct on /src/contracts/config.ts**
...
This script reuses the existing public token addresses and only redeploys `p.tokens` and bridges, then updates `src/contracts/config.ts`.
...
```bash
git diff src/contracts/config.ts
```

### Regenerate flattened files and verification JSONs

```bash
npx hardhat run scripts/generate_all_flattened_and_json.cjs
...
```bash
git add src/contracts/config.ts
