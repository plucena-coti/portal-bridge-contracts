...
async function main() {
    ...
    const bridgeCoti = await deployBridge("PrivacyBridgeCotiNative", "contracts/privacyBridge/PrivacyBridgeCotiNative.sol:PrivacyBridgeCotiNative", [await pCoti.getAddress()]);

    // 2. WETH
    const weth = await deployPublic("WETH", "WETH9");
    const pWeth = await deployPrivate("PrivateWETH", "PrivateWrappedEther");
    const bridgeWeth = await deployBridge("PrivacyBridgeWETH", "contracts/privacyBridge/PrivacyBridgeWETH.sol:PrivacyBridgeWETH", [await weth.getAddress(), await pWeth.getAddress()]);

    // 3. WBTC
    const wbtc = await deployPublic("WBTC", "WBTC");
    const pBtc = await deployPrivate("PrivateBTC", "PrivateWrappedBTC");
    const bridgeBtc = await deployBridge("PrivacyBridgeWBTC", "contracts/privacyBridge/PrivacyBridgeWBTC.sol:PrivacyBridgeWBTC", [await wbtc.getAddress(), await pBtc.getAddress()]);

    // 4. USDT
    const usdt = await deployPublic("USDT", "USDT");
    const pUsdt = await deployPrivate("PrivateUSDT", "PrivateTetherUSD");
    const bridgeUsdt = await deployBridge("PrivacyBridgeUSDT", "contracts/privacyBridge/PrivacyBridgeUSDT.sol:PrivacyBridgeUSDT", [await usdt.getAddress(), await pUsdt.getAddress()]);

    // 5. USDC.e
    const usdc = await deployPublic("USDCe", "USDCe");
    const pUsdc = await deployPrivate("PrivateUSDCe", "PrivateBridgedUSDC");
    const bridgeUsdc = await deployBridge("PrivacyBridgeUSDCe", "contracts/privacyBridge/PrivacyBridgeUSDCe.sol:PrivacyBridgeUSDCe", [await usdc.getAddress(), await pUsdc.getAddress()]);

    // 6. WADA
    const wada = await deployPublic("WADA", "WADA");
    const pWada = await deployPrivate("PrivateWADA", "PrivateWrappedADA");
    const bridgeWada = await deployBridge("PrivacyBridgeWADA", "contracts/privacyBridge/PrivacyBridgeWADA.sol:PrivacyBridgeWADA", [await wada.getAddress(), await pWada.getAddress()]);
    ...
    const bridgeGCoti = await deployBridge("PrivacyBridgegCoti", "contracts/privacyBridge/PrivacyBridgegCoti.sol:PrivacyBridgegCoti", [await gCoti.getAddress(), await pGCoti.getAddress()]);
